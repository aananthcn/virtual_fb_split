...
static unsigned long fbZero_phy = 0;
static unsigned long fbZero_vir = 0;
static unsigned long fbZero_len = 0;
static unsigned long fbZero_bpp = 0;
static unsigned long fbNext_phy = 0;
static unsigned long fbNext_vir = 0;
...
static int virtfb_map_video_memory(struct fb_info *fbi)
{
	...
	if (fb_no == 0) {
		...
		/* take back up of fb0 */
		fbZero_vir = (unsigned long) fbi->screen_base;
		fbZero_phy = fbi->fix.smem_start;
		fbZero_len = fbi->fix.smem_len;
		fbZero_bpp = fbi->var.bits_per_pixel;

		/* prepare for display splitting */
		fbNext_vir = fbZero_vir;
		...
		fbNext_phy = fbi->fix.smem_start + fbi->fix.smem_len;
	}
...
static int virtfb_register(struct fb_info *fbi, unsigned int id, int xres, int yres)
{
...
int __init allocate_register_fb(struct fb_info **fbi, int i, int x, int y)
{
	u32 *  fbNum;
	int ret = 0;

	*fbi = virtfb_init_fbinfo(&virtfb_ops);
	if (!*fbi) {
		ret = -ENOMEM;
		return ret;
	}
